...
def guardar_traduccion(carpeta_salida, nombre_archivo, glosas, traduccion):
    os.makedirs(carpeta_salida, exist_ok=True)
    nombre_salida = nombre_archivo.replace('.txt', '_traduccion.txt')
    ruta_salida = os.path.join(carpeta_salida, nombre_salida)
    with open(ruta_salida, 'w', encoding='utf-8') as f:
        f.write(f"Archivo origen : {nombre_archivo}\n")
        f.write(f"Fecha          : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Glosas LSM     : {glosas}\n")
        f.write(f"Traduccion     : {traduccion}\n")
    return ruta_salida
...
if __name__ == "__main__":
    ...
    if not datos_vision:
        ...
        for dato in datos_vision_ordenados:
            ...
            ruta = guardar_traduccion(carpeta_traducciones, dato['archivo'], dato['glosas'], resultado)
